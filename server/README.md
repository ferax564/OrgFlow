...
The stdio bridge supports newline-delimited JSON-RPC and explicit proposal opt-in. The HTTP `/api/mcp` endpoint implements the documented request/response subset, not full Streamable HTTP session/SSE support. Use the stdio bridge for clients requiring that transport; do not advertise generic remote MCP interoperability without testing the target client. Notifications receive no JSON-RPC reply. Never expose an API token in a public browser bundle.
